from typing import Dict, AnyStr, List
from pydantic import BaseModel, Field
from ..schemas.user_schema import UserSchema, UserMinimalModel
from ..providers import project_db
from ..utils.utils import get_current_time


class ProjectModel(BaseModel):
    project_id: str = Field(..., title="Project ID")
    project_name: str = Field(..., title="Project Name")
    project_alias: str = Field(..., title="Project Alias")
    project_description: str = Field(..., title="Project Description")
    created_at: str = Field(..., title="Project Created At")
    last_opened: str = Field(..., title="Project Last Opened")


class ProjectSchema:
    '''
    Schema and Validation for Project.
    '''

    def __init__(
        self,
        project_id: AnyStr = None,
        project_name: AnyStr = "",
        project_alias: AnyStr = "",
        project_description: AnyStr = "",
        created_at: AnyStr = get_current_time(),
        last_opened: AnyStr = get_current_time(),
    ):
        self.project_id = project_id
        self.project_name = project_name
        self.project_alias = project_alias
        self.project_description = project_description
        self.created_at = created_at
        self.last_opened = last_opened

    def to_dict(self, include_id=True):
        data_dict = {
            "project_name": self.project_name,
            "project_alias": self.project_alias,
            "project_description": self.project_description,
            "created_at": self.created_at,
            "last_opened": self.last_opened,
        }
        if include_id:
            data_dict["project_id"] = self.project_id
        return data_dict

    @staticmethod
    def from_dict(data: Dict):
        return ProjectSchema(
            project_id=data.get("project_id"),
            project_name=data.get("project_name"),
            project_alias=data.get("project_alias"),
            project_description=data.get("project_description"),
            created_at=data.get("created_at"),
            last_opened=data.get("last_opened"),
        )

    @staticmethod
    def find_by_alias(alias: AnyStr):
        # Get in cache
        queries = project_db.cacher.get(
            f"{project_db.collection_name}:{alias}")
        if not queries:
            queries = project_db.query_equal("project_alias", alias)
            if len(queries) == 0:
                return None
            # Save to cache
            project_db.cacher.set(
                f"{project_db.collection_name}:{alias}", queries)
        return ProjectSchema.from_dict(queries[0])

    @staticmethod
    def find_by_id(project_id: AnyStr):
        data = project_db.get_by_id(project_id)
        if not data:
            return None
        return ProjectSchema.from_dict(data)

    @staticmethod
    def find_all_by_ids(project_ids: List[AnyStr]):
        projects = project_db.get_all_by_ids(ids=project_ids)
        return [ProjectSchema.from_dict(project) for project in projects]

    def create_project(self):
        project_id = project_db.create(self.to_dict(include_id=False))
        self.project_id = project_id
        self.created_at = get_current_time()
        # Add data to cache
        project_db.cacher.set(
            f"{project_db.collection_name}:{project_id}", self.to_dict(include_id=True))
        return self

    def update_project(self, data):
        project_db.update(self.id, data)

    # def update_members(self, members: List[AnyStr], is_add: bool = True):
    #     if is_add:
    #         self.members = list(set(self.members) | set(members))
    #     else:
    #         self.members = list(set(self.members) - set(members))
    #     # Add data to cache
    #     project_db.update(self.id, {"members": self.members})

    def delete_project(self):
        project_db.delete(self.id)

    # def update_positions(self, positions_id: AnyStr, is_add: bool):
    #     if is_add:
    #         self.positions.append(positions_id)
    #     else:
    #         self.positions.remove(positions_id)
    #     # Add data to cache
    #     project_db.update(self.id, {"positions": self.positions})
    #     return self
